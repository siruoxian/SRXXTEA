...
static uint32_t * srxxtea_to_uint_array(const uint8_t * data, size_t len, int inc_len, size_t * out_len) {
    uint32_t *out;
    size_t n;
...
static uint8_t * srxxtea_to_ubyte_array(const uint32_t * data, size_t len, int inc_len, size_t * out_len) {
    uint8_t *out;
    size_t m, n;
...
static uint32_t * srxxtea_uint_encrypt(uint32_t * data, size_t len, uint32_t * key) {
...
static uint32_t * srxxtea_uint_decrypt(uint32_t * data, size_t len, uint32_t * key) {
    uint32_t n = (uint32_t)len - 1;
...
static uint8_t * srxxtea_ubyte_encrypt(const uint8_t * data, size_t len, const uint8_t * key, size_t * out_len, int sign) {
    uint8_t *out;
    ...
    //  无符号位 0 有符号位1
    data_array = srxxtea_to_uint_array(data, len, sign, &data_len);
    if (!data_array) return NULL;
    
    key_array  = srxxtea_to_uint_array(key, 20, 0, &key_len);
    if (!key_array) {
        free(data_array);
        return NULL;
    }
    
    out = srxxtea_to_ubyte_array(srxxtea_uint_encrypt(data_array, data_len, key_array), data_len, 0, out_len);
...
static uint8_t * srxxtea_ubyte_decrypt(const uint8_t * data, size_t len, const uint8_t * key, size_t * out_len, int sign) {
    uint8_t *out;
    uint32_t *data_array, *key_array;
    size_t data_len, key_len;
    ...
    out = srxxtea_to_ubyte_array(srxxtea_uint_decrypt(data_array, data_len, key_array), data_len, 1, out_len);
...
+ (NSData *) encrypt:(NSData *)data key:(NSData *)key sign:(BOOL)sign {
    ...
    void * bytes = srxxtea_ubyte_encrypt(data.bytes, data.length, fixed_key, &out_len, s);
...
+ (NSData *) decrypt:(NSData *)data key:(NSData *)key sign:(BOOL)sign {
    ...
    void * bytes = srxxtea_ubyte_decrypt(data.bytes, data.length, fixed_key, &out_len ,s);
...
@implementation NSData (SRXXTEA)

- (NSData *) srxxteaEncrypt:(NSData *)key sign:(BOOL)sign {
    return [SRXXTEA encrypt:self key:key sign:sign];
}

- (NSData *) srxxteaDecrypt:(NSData *)key sign:(BOOL)sign {
    return [SRXXTEA decrypt:self key:key sign:sign];
}
